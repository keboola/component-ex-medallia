...
* **HTTP-free failure cases** (``01``–``06``) — config-validation / raw-contract failures that
  return exit 1 before any HTTP call (empty cassette). Scaffolded from
  ``tests/setup/configs.failure.json`` with NO secrets and no network.
* **Live-recorded data / sync cases** (``07``+) — recorded once against the live instance from
